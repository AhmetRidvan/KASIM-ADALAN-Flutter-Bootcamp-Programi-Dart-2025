...
class KisilerdaoRepository {
  ...
  Future<List<Kisiler>> research(String text) async {
    var db = await VeritabaniYardimcisi.veritabaniErisim();
    List<Map<String, dynamic>> listOfMaps = await db.rawQuery(
      "SELECT * FROM kisiler WHERE kisi_ad like '%$text%'",
    );
    return List.generate(listOfMaps.length, (index) {
      var herbirMap = listOfMaps[index];
      return Kisiler(
        kisiId: herbirMap['kisi_id'],
        kisiAd: herbirMap['kisi_ad'],
        kisiTel: herbirMap['kisi_tel'],
      );
    });
  }
}
